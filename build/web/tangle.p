...
ASCIIcode=byte;{:11}{12:}textfile=Text;{:12}{37:}
eightbits=0..255;sixteenbits=0..65535;{:37}{39:}namepointer=0..maxnames;
UNIXfile_name=packed array[1..60] of char;
...
webname,changename,Pascalname,poolname:UNIXfile_name;{:189}
...
var dotpos,i,a:integer;c:char;fname:UNIXfile_name;
...
scanargs;Assign(Pascalfile, trim(Pascalname)); Rewrite(Pascalfile);Assign(pool, trim(poolname)); Rewrite(pool);{:26}
...
{:180}end;{:2}{24:}procedure openinput;begin Assign(webfile, trim(webname)); Reset(webfile);
Assign(changefile, trim(changename)); Reset(changefile);end;{:24}{28:}
...
else write(output,xchr[a])end[:76];k:=k+1;end;end;end;}{:74}{84:}
...
else controlcode:=0 end;end;{:139}{140:}
...
else write(output,'?')end;end;end;10:end;}{:181}{182:}
...
writeln(output);if(history<>0)and(history<>1)then Halt(1)else Halt(0);
end.{:182}
