...
stacksize=50;maxidlength=50;unambiglength=7;
{:8}type{11:}ASCIIcode=ByteCard;{:11}{37:}eightbits=0..255;
...
{:78}{190:}UNIXfilename=packed array[1..60]of char;
{:190}var{9:}history:0..3;
{:9}{13:}xord:external array[char]of ASCIIcode;
xchr:external array[ASCIIcode]of char;{:13}{23:}webfile:text;
changefile:text;{:23}{25:}Pascalfile:text;pool:text;
{:25}{27:}buffer:external array[0..bufsize]of ASCIIcode;
...
{:185}{189:}webname,changename,Pascalname,poolname:UNIXfilename;
{:189}
#include "tangext.h"
...
poolname[dotpos+5]:=chr(0);foundweb:=true;
end{:192}else if not foundchange then{193:}begin dotpos:=-1;i:=1;
...
error;jumpout;end{:196};
end else{195:}begin{196:}begin begin writeln(output);
write(output,'! Usage: tangle webfile[.web] [changefile[.ch]]');end;
error;jumpout;end{:196};end{:195};end;
if not foundweb then{196:}begin begin writeln(output);
write(output,'! Usage: tangle webfile[.web] [changefile[.ch]]');end;
error;jumpout;end{:196};
if not foundchange then{194:}begin changename[1]:='/';
changename[2]:='d';changename[3]:='e';changename[4]:='v';
changename[5]:='/';changename[6]:='n';changename[7]:='u';
changename[8]:='l';changename[9]:='l';changename[10]:=chr(0);end{:194};
end;{:191}procedure initialize;var{16:}i:0..255;{:16}{41:}wi:0..2;
...
{:18}{21:}{:21}{26:}scanargs;rewrite(Pascalfile,Pascalname);
...
reset(changefile,changename);end;
...
otherwise write(output,xchr[a])end[:76];k:=k+1;end;end;end;}
...
write(output,'! Two numbers occurred without a sign between them');
error;end;end;{:108}otherwise goto 666 end;goto 10;
...
otherwise controlcode:=0 end;end;
{:139}{140:}function skipahead:eightbits;label 30;var c:eightbits;
...
writeln(output,'This is TANGLE, Version 4.6 for Linux');
{183:}phaseone:=true;modulecount:=0;repeat nextcontrol:=skipahead;
until nextcontrol=136;while not inputhasended do scanmodule;
...
write(output,'Writing the output file');end;flushstdout;
