{2:}{4:}{:4}program WEAVE(input,output);label 9999;
...
maxtoks=30000;maxtexts=2000;maxscraps=1000;stacksize=400;
{:8}type{11:}ASCIIcode=ByteCard;{:11}{36:}eightbits=0..255;
...
tokfield:sixteenbits;modefield:mode;end;
{:201}{266:}UNIXfilename=packed array[1..60]of char;
{:266}var{9:}history:0..3;
{:9}{13:}xord:external array[char]of ASCIIcode;
xchr:external array[ASCIIcode]of char;{:13}{23:}webfile:text;
changefile:text;{:23}{25:}texfile:text;
{:25}{27:}buffer:external array[0..longbufsize]of ASCIIcode;
...
{:48}{53:}tokmem:array[0..maxtoks]of sixteenbits;
...
{:258}{265:}webname,changename,texname:array[1..60]of char;
noxref:boolean;
{:265}
#include "tangext.h"
...
{:33}{267:}procedure scanargs;var dotpos,i,a:integer;fname:UNIXfilename;
...
texname[dotpos+4]:=chr(0);foundweb:=true;
end{:268}else if not foundchange then{269:}begin dotpos:=-1;i:=1;
...
true;i:=i+1;end;end{:271};end;
if not foundweb then{272:}begin begin writeln(output);
write(output,'! Usage: weave [-x] webfile[.web] [changefile[.ch]]');end;
error;jumpout;end{:272};
if not foundchange then{270:}begin changename[1]:='/';
changename[2]:='d';changename[3]:='e';changename[4]:='v';
changename[5]:='/';changename[6]:='n';changename[7]:='u';
changename[8]:='l';changename[9]:='l';changename[10]:=chr(0);end{:270};
end;{:267}procedure initialize;var{16:}i:0..255;{:16}{40:}wi:0..1;
...
{:18}{21:}{:21}{26:}scanargs;rewrite(texfile,texname);
...
begin reset(webfile,webname);reset(changefile,changename);end;
{:24}{28:}function inputln(var f:text):boolean;begin limit:=0;
...
{:88}otherwise begin begin if not phaseone then begin writeln(output);
...
otherwise write(output,'UNKNOWN')end;end;}
...
21:if(nextcontrol=129)or(nextcontrol=2)then{189:}begin tokmem[tokptr]:=
92;tokptr:=tokptr+1;if nextcontrol=2 then begin tokmem[tokptr]:=61;
...
cat[scrapptr]:=1;trans[scrapptr]:=textptr;textptr:=textptr+1;
tokstart[textptr]:=tokptr;end;
end{:189}else if(nextcontrol=130)then{191:}begin p:=idlookup(0);
...
textptr:=textptr+1;tokstart[textptr]:=tokptr;end;
begin scrapptr:=scrapptr+1;cat[scrapptr]:=3;trans[scrapptr]:=0;end;end;
{:193}otherwise begin nextcontrol:=ilk[p]-24;goto 21;end end;
end{:191}else if(nextcontrol=134)then{190:}begin tokmem[tokptr]:=92;
...
otherwise write(output,'?')end;end;end;10:end;}
{:260}{261:}procedure PhaseI;begin{109:}phaseone:=true;
phasethree:=false;resetinput;modulecount:=0;changedmodule[0]:=false;
skiplimbo;changeexists:=false;
...
otherwise end;until nextcontrol>=143{:113};
...
end;end;otherwise end;until nextcontrol>=143{:222};
...
writeln(output,'This is WEAVE, Version 4.5 for Linux');{64:}idloc:=10;
...
end{:85};9999:{262:}begin writeln(output);
